...
3. **Single Runtime Boundary**: local application behavior lives in the supported Tauri process; removed implementations remain available through Git history
...
Current app behavior belongs in the Rust/Tauri core. Add frontend-facing behavior through Tauri commands/events and existing Rust services under `frontend/src-tauri/src`.
...
4. **Single Runtime Boundary**: Meeting persistence, transcription, and LLM features are handled by the Tauri app. A separate service begins only as an explicitly activated deployment work package.
