...
mod tests {
    ...
    async fn test_list_system_audio_devices() {
        // Device availability is host-dependent; the command boundary should
        // complete without panicking on both workstation and CI hosts.
        let _ = list_system_audio_devices_command().await;
    }
