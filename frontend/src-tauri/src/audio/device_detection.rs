...
pub fn calculate_buffer_timeout(
    ...
    // Calculate base timeout with 2x jitter headroom using integer nanoseconds.
    // Integer arithmetic keeps exact frame-rate ratios deterministic across
    // platforms and avoids float rounding at timeout boundaries.
    let timeout_nanos = u64::from(buffer_size) * 2_000_000_000 / u64::from(sample_rate);
    let with_headroom = Duration::from_nanos(timeout_nanos);
